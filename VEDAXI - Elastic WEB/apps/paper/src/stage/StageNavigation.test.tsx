...
describe("Semantic Stage navigation", () => {
  ...
  it("keeps the destination heading anchored until native Tab continuation resolves", () => {
    let focusedId = "mobile-chapter-link";
    let focusOptions: FocusOptions | undefined;
    let focusable = false;
    let onBlur: (event: KeyboardEvent) => void = () => undefined;
    let onKeyDown: (event: KeyboardEvent) => void = () => undefined;
    const targets = new Map([
      ["methods-title", {
        addEventListener: (type: "blur" | "keydown", listener: (event: KeyboardEvent) => void) => {
          if (type === "blur") onBlur = listener;
          else onKeyDown = listener;
        },
  ...
  it("commits the keyboard hash before handing focus to the destination", () => {
    ...
    const documentRef = {
      getElementById: (id: string) => id === "methods-title" ? {

...
interface ChapterFocusDocument {
  getElementById(id: string): {
    addEventListener(type: "blur" | "keydown", listener: (event: KeyboardEvent) => void, options?: { once: true }): void;
    focus(options?: FocusOptions): void;
    removeEventListener(type: "keydown", listener: (event: KeyboardEvent) => void): void;
    removeAttribute(name: "tabindex"): void;
    setAttribute(name: "tabindex", value: "-1"): void;
  } | null;
}
...
export function handleStageChapterActivation(
  ...
  const continueFromHeading = (event: KeyboardEvent) => {
    if (event.key === "Tab") target.removeAttribute("tabindex");
  };
  const cleanup = () => {
    target.removeAttribute("tabindex");
    target.removeEventListener("keydown", continueFromHeading);
  };
  target.setAttribute("tabindex", "-1");
  target.addEventListener("keydown", continueFromHeading);

...
export type PaperTools = readonly [WebMcpTool, ...WebMcpTool[]];
...
export class PaperRegistrationController implements PaperProtocolControls {
  ...
  constructor(
    private readonly tools: PaperTools,
  ...
  enable = () => {
    ...
    void this.register([...this.tools], [], { lifecycleSignal: nextLifecycle.signal })
...
export function usePaperRegistration(tools: PaperTools): PaperProtocolControls {
  const [status, setStatus] = useState<PaperProtocolStatus>("checking");
  const controller = useMemo(() => new PaperRegistrationController(tools, setStatus), [tools]);
